...
class RadarCacheHelper(
    ...
    fun updateMapCachePolicy(policy: MapCachePolicy) {
        ...
        scope.launch { repository.saveAppSetting(AppStateSetting(key = "map_cache_policy", stringValue = policy.name)) }
    ...
    fun updateLocationSyncProfile(profile: LocationSyncPowerProfile, syncer: BatteryAwareLocationSyncer?) {
        ...
        scope.launch { repository.saveAppSetting(AppStateSetting(key = "location_sync_profile", stringValue = profile.name)) }
