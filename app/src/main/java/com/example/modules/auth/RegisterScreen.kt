...
fun RegisterScreen(
    ...
    Column(
        ...
        OutlinedTextField(
            value = plate,
            onValueChange = {
                var clean = it.uppercase()
                if (!clean.startsWith("N")) { clean = "N " + clean.trim() }
                plate = clean
            },
            label = { Text("Nomor Pelat Motor (Jatim Malang)") },
            modifier = Modifier.fillMaxWidth().testTag("reg_plate"),
        ...
        OutlinedTextField(
            ...
            placeholder = { Text("Contoh: Honda Vario 160") },
            modifier = Modifier.fillMaxWidth().testTag("reg_motor"),
            shape = RoundedCornerShape(12.dp)
        )
        ...
        Button(
            onClick = {
                if (name.isNotBlank() && phone.isNotBlank() && plate.length > 2 && model.isNotBlank()) {
                    ...
                    onSubmit(name, phone, plate, model, "Malang")
                }
            },
            enabled = name.isNotBlank() && phone.isNotBlank() && plate.length > 2 && model.isNotBlank(),
            modifier = Modifier.fillMaxWidth().height(52.dp).testTag("reg_submit_button"),
